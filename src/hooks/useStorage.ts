 ...
 * @param opts.fastCache When true, mirrors the value into a synchronous
 * localStorage cache (`lib/fastCache.ts`, keyed `sg:fast:<key>`) and reads it
 * back on mount BEFORE `storage.sync` resolves — the same first-paint
 * fast-path `BackgroundContext` uses, so callers whose data other widgets
 * depend on (e.g. the widget list) render with last-known-good data
 * immediately on every new tab instead of blocking on a `storage.sync`
 * round trip. Cross-device sync is unaffected — `storage.sync` stays the
 * source of truth, this is purely a local read-ahead cache.
 ...
 */
export function useStorage<T>(key: string, defaultValue: T, opts?: { fastCache?: boolean }) {
  const fastCache = useRef(opts?.fastCache ? createFastCache<T>(`sg:fast:${key}`, { json: true }) : null);
  const [value, setValue] = useState<T>(() => fastCache.current?.read() ?? defaultValue);
  ...
  useEffect(() => {
    storage.get(key).then((stored) => {
      if (stored !== undefined && stored !== null) {
        ...
        fastCache.current?.write(stored as T);
      }
      setLoaded(true);
  ...
  useEffect(() => {
    ...
    // Fast-cache write is immediate (cheap, unlike storage.sync's write-rate
    // limit) so the very next new tab sees this change right away, even
    // before the debounced storage.sync write below has landed.
    fastCache.current?.write(value);
  ...
  useEffect(() => {
    return storage.addChangeListener((changedKey, newValue) => {
      ...
      fastCache.current?.write(newValue as T);
